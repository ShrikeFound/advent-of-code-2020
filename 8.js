...
const gameConsole = (() => {
  ...
  // console.log("====================playing===========")
    ...
    for (line = 0; line < instructions.length; line++){
      // console.log(line," : ",instructions[line]);
      if (memory.includes(line)) {
        // console.log(line, " is duplicate");
        // console.log(accumulator);
        return accumulator;
      }
      memory.push(line);
...
const debug = () => {
  for (let i = 0; i < instructions.length; i++){
    let output =instructions[i]
    if ((/(jmp)/).test(instructions[i])) {
      instructions[i] = instructions[i].replace("jmp", "nop")
      output += " replacing with "+instructions[i]
      gameConsole.play(instructions);
      instructions[i] = instructions[i].replace("nop", "jmp")
    } else if ((/(nop)/).test(instructions[i]) && Number(instructions[i].slice(instructions[i].indexOf(" "))) != 0) {
